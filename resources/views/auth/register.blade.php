        ...
        <div class="hidden md:block md:w-1/2 2xl:w-1/2 bg-purple-500">
            <div class="2xl:px-52 2xl:py-52 md:px-20 md:py-32">
                <div class="grid gap-3">
                    <span class="2xl:text-4xl md:text-3xl font-bold text-white">Create An Account Today!</span>
        ...
        <div class="md:w-1/2 2xl:w-1/2">
            <div class="grid h-screen place-items-center">
                <form action="{{ route('register') }}" method="POST"
                    ...
                    <div class="grid gap-2">
                        <label for="" class="text-lg font-semibold">Name</label>
                    ...
                    <div class="grid gap-2">
                        <label for="" class="text-lg font-semibold">Package</label>
                        <select name="package" id="" class="border rounded-md p-3 focus:outline-purple-500">
                            @foreach ($packages as $package)
                                <option value="{{ $package->amount }}">{{ $package->name }}</option>
                            @endforeach
                        </select>
                        @error('email')
                            <span class="text-red-500">
                    ...
                    <div>
                        <button type="submit" class="bg-purple-500 text-white px-5 py-3 rounded-md w-full">Create
                            Account</button>
                    </div>
                    <div>
                        <a href="{{ route('show.login') }}" class="hover:text-purple-500 hover:underline">Already own an
                            account? Login</a>
                    </div>
                </form>
            </div>
